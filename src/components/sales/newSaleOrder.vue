<template>
    <div>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>新增销售订单</el-breadcrumb-item>
        </el-breadcrumb>
        <el-form>
                <el-card>主单据信息</el-card>
                <div class="mainInfo">
                    <div class="allegation mainInfo-grid">
                        <span class="title">销售单编号</span>
                        <el-input v-model="newOrder.soId" size="small" class="el-inner"></el-input>
                    </div>
                    ...
                    <div class="supplier mainInfo-grid">
                        ...
                        <el-select v-model="customer.name" class="el-inner" placeholder="客户" @change="selectCustomer($event)" style="width:150px" size="small">
                            <el-option v-for="c in customer" :key="c.customerCode" :label="c.name" :value="c.name"></el-option>
                        </el-select>
                    </div>
                    ...
                    <div class="productTotal mainInfo-grid">
                        <span class="title">销售产品总价</span><el-input size="small" class="productTotal" v-model="newOrder.productTotal"></el-input>
                    </div>
                    <div class="poTotal mainInfo-grid">
                        <span class="title">销售单总价格</span><el-input size="small" class="poTotal" v-model="newOrder.soTotal"></el-input>
                    </div>
                    ...
                    <div class="mainInfo-grid">
                        <span class="title">销售单状态</span>
                        <el-select v-model="status.value" class="el-inner" placeholder="销售单状态" @change="selectStatus($event)" style="width:150px" size="small">
        ...
        <el-card><span>销售单明细</span><span @click="addDetail"><i class="el-icon-circle-plus">添加明细</i></span></el-card>
        <table>
            <tr>
...
export default {
    data(){
        return{
           ...
           newOrder:
               {
                soId:'',        //Number   销售单编号。要求：唯一，不能为空。   必须
                customerCode :'', //String   客户编号。要求：不能为空。   必须
                ...
                soTotal:0,         //Number   销售单总价。要求：不能为负数。   必须
                payType : '',      //Number   付款方式。   必须 1  货到付款2   款到发货3   预付款到发货,       
                prePayFee:0,       //Number   最低预付款金额。要求：不能为负数，默认为0。   必须
                status:'',          //Number   销售单状态。   目前有的值如下：（必须） 1   新增 2   已发货 3   已收款 4   已了结 5   已预付
                remark:'',          //String   备注。   可选         
               },
               //付款方式
               payType : [              //Number   付款方式。   必须 1  货到付款2   款到发货3   预付款到发货
                ...
                status:[
                    ...
                    {type:2,value:2,label:'已发货'},
                    {type:3,value:3,label:'已收款'},
    ...
    methods:{
        ...
        //获取所有客户
        getAllCustomer(){
            Axios.get('/cgi/main/sell/customer/show').then(res=>{
        ...
        selectCustomer(customer){
            let cus = {};
            cus = this.customer.find((item)=>{
                return item.name === customer
            })
            this.newOrder.customerCode = cus.customerCode;
        },
        //输入数量
        enterNum(m,i){
        ...
        //销售总价
        getPoTotal(){
            this.newOrder.soTotal =Number(this.newOrder.tipFee)+Number(this.newOrder.productTotal);
        },
        //保存单据
        saveAll(){
            Axios({
                ...
                data:JSON.stringify({
                    soId : this.newOrder.soId,
                    customerCode : this.newOrder.customerCode,
                ...
                if(res.data.code===2){
                    ...
                    this.$router.push({path:'/addSalesList'})
                }else{
